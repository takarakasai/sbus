...
- **未知のスロットは捨てず `Telemetry::Unknown` にする。** 黙って誤デコードするより
  上位でカウントできるほうが安全。

  > **この判断は 2026-08-21 に報われた。** 当時未検証だった「25.5 V 超で marker が
  > 変化する可能性」は実際に起きた（`doc/spec.md` §5.2 の訂正）。捨てる実装なら
  > Ext-Volt が古い値のまま固まるだけだが、`Unknown` としてカウントする実装
  > だったため `unknown_slots` の増加として観測できる状態になっていた。
...
および将来の異常キャプチャ（failsafe や高電圧域）を後から解析できること。
